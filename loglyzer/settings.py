...
LOG_FILE_FORMAT = {
    'log_format': r'^(?P<remote_addr>[0-9a-f.:]*) - (?P<remote_user>[^\S]*) \[(?P<time_local>[^]]*)\] "(?P<request>[^"]*)" (?P<status>\d*) (?P<body_bytes_sent>\d*) "(?P<http_referer>[^"]*)" "(?P<http_user_agent>[^"]*)" "(?P<http_x_forwarded_for>[^"]*)"$',
